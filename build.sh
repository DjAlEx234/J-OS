...
i686-elf-gcc -c ./Interface/interface.c -o ./Output/interface.o -std=gnu99 -ffreestanding -O2 -Wall -Wextra $flags -I"./Headers/"
i686-elf-gcc -c ./Interface/string.c -o ./Output/string.o -std=gnu99 -ffreestanding -O2 -Wall -Wextra $flags -I"./Headers/"
nasm -felf32 ./ASM/int.asm -o ./Output/intasm.o
nasm -felf32 ./Boot/boot.asm -o ./Output/boot.o
cd Output
i686-elf-gcc -T ../Boot/linker.ld -o ../J-OS$debug.bin -I"../Headers/" -ffreestanding -O2 -nostdlib int.o intasm.o inoutb.o entry.o boot.o text.o key.o string.o vga.o interface.o serial.o cmd.o power.o terminal.o -lgcc
cd ..
